...
import type { TennisFormMode } from '@/lib/tennis-forms/shared';

export function useTennisSectionState<T extends object>(
  emptyForm: T,
  options?: { onDataChanged?: () => void },
) {
  const toast = useToast();
  const queryClient = useQueryClient();
  const [mode, setMode] = useState<TennisFormMode>('create');
...
export function submitLabelForMode(mode: TennisFormMode): string {
  switch (mode) {
    case 'create':
      return 'Crear';
    case 'edit':
